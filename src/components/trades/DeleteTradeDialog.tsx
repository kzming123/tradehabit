    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="bg-[#0e1223] border-[#1e293b] text-[#f8fafc] max-w-sm w-full p-0 gap-0">
        <DialogHeader className="px-6 pt-6 pb-4">
          ...
          <DialogTitle className="text-[15px] font-bold">{t("portfolio.deleteTradeTitle")}</DialogTitle>
          <p className="text-[13px] text-[#94a3b8] mt-1.5 leading-relaxed">
            {t("portfolio.deleteTradeConfirm")}{" "}
            <span className="text-[#f8fafc] font-semibold">{tradePair}</span>{" "}
            {t("portfolio.deleteTradeSuffix")}
          </p>
        </DialogHeader>
