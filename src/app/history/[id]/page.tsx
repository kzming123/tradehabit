...
function EmoBadge({ emotion }: { emotion: string }) {
  ...
  return (
    <span className={cn("text-[12px] font-semibold border px-2.5 py-1 rounded-lg", m.color, m.bg)}>
      {m.emoji} {m.label}
    </span>
  );
}
...
export default function TradeDetailPage() {
  ...
  useEffect(() => {
    const t = tradeStorage.getById(id);
    if (!t) { router.replace("/history"); return; }
    setTrade(t);
    const p = portfolioStorage.getById(t.portfolioId);
    setPortfolio(p ?? null);
  }, [id, router]);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/history"
          className="flex items-center gap-1.5 text-[12px] font-semibold text-[#475569] hover:text-[#f8fafc] transition-colors"
        >
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/add-trade?edit=${trade.id}`}
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg border border-[#1e293b] text-[12px] font-semibold text-[#475569] hover:border-[#334155] hover:text-[#f8fafc] transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg border border-[#ef4444]/30 text-[12px] font-semibold text-[#ef4444] hover:bg-[#ef4444]/10 transition-colors cursor-pointer"
          >
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span
                className={cn(
                  "text-[11px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-md",
            ...
            <p className="text-[12px] text-[#475569] mt-1">
              {portfolio?.name ?? "—"} · {trade.market} · {formatDateTime(trade.dateTime)}
            </p>
          </div>
          <div className="text-right">
            <p className={cn("text-[26px] font-bold tabular leading-none tracking-[-0.02em]", pnlColor)}>
              {trade.pnl >= 0 ? "+" : ""}${formatNum(Math.abs(trade.pnl))}
            </p>
            <p className={cn("text-[13px] font-semibold tabular mt-0.5", pnlColor)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Section title="Setup Tag">
          {trade.setupTag ? (
            <span className="inline-flex items-center px-3 py-1.5 rounded-lg bg-[#0f172a] border border-[#1e293b] text-[13px] font-semibold text-[#f8fafc]">
              {trade.setupTag}
            </span>
          ) : (
        ...
        <Section title="Emotions">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              {trade.emotionBefore ? (
                <EmoBadge emotion={trade.emotionBefore} />
              ) : (
            ...
            <div className="flex items-center gap-2">
              ...
              {trade.emotionAfter ? (
                <EmoBadge emotion={trade.emotionAfter} />
              ) : (
                <span className="text-[12px] text-[#334155]">—</span>
              )}
            </div>
        ...
        {trade.mistakes.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {trade.mistakes.map((m) => (
              <span
                key={m}
                className="text-[12px] font-semibold text-[#f59e0b] bg-[#f59e0b]/10 border border-[#f59e0b]/20 px-2.5 py-1 rounded-lg"
              >
                {MISTAKE_LABELS[m] ?? m}
              </span>
            ))}
      ...
      {(trade.notes || trade.lessonLearned) && (
        <Section title="Reflection">
          <div className="space-y-4">
            {trade.notes && (
              <div>
                <p className="text-[10px] font-bold text-[#334155] uppercase tracking-[0.08em] mb-1.5">
                  Trade Notes
                </p>
      ...
      <Section title="Details">
        <div>
          <DataRow label="Portfolio">
            <span className="text-[13px] font-semibold text-[#f8fafc]">{portfolio?.name ?? "—"}</span>
          </DataRow>
          <DataRow label="Market">
            <span className="text-[13px] text-[#94a3b8]">{trade.market}</span>
          </DataRow>
          <DataRow label="Logged">
            <span className="text-[13px] text-[#94a3b8]">
              {new Intl.DateTimeFormat("en-US", { dateStyle: "medium", timeStyle: "short", hour12: false }).format(new Date(trade.createdAt))}
            </span>
          </DataRow>
          <DataRow label="Trade ID">
            <span className="text-[11px] text-[#334155] font-mono">{trade.id}</span>
          </DataRow>
        </div>
      </Section>
      ...
      <div className="flex gap-3 pb-4">
        <Link
          href={`/add-trade?edit=${trade.id}`}
          className="flex-1 h-11 rounded-xl border border-[#1e293b] text-[14px] font-semibold text-[#475569] flex items-center justify-center gap-2 hover:border-[#334155] hover:text-[#f8fafc] transition-colors cursor-pointer"
        >
        ...
        <button
          onClick={() => setDeleteOpen(true)}
          className="flex-1 h-11 rounded-xl border border-[#ef4444]/30 text-[14px] font-semibold text-[#ef4444] flex items-center justify-center gap-2 hover:bg-[#ef4444]/10 transition-colors cursor-pointer"
        >
